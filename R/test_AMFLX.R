...
ppfd_sun <- irradiance_partition("sun",data_flx$timestamp, data_flx$PPFD, 
                                 data_flx$LAI, data_flx$PA, 
                                 PA0 = 101325, fa = 0.426, canopy_reflexion = 0.15, 
                                 rho_cd = 0.036, kd_prime = 0.719, 
                                 data_flx$lat, data_flx$long)

ppfd_shade <- irradiance_partition("shade",data_flx$timestamp, data_flx$PPFD, 
                                   data_flx$LAI, data_flx$PA, 
                                   PA0 = 101325, fa = 0.426, canopy_reflexion = 0.15, 
                                   rho_cd = 0.036, kd_prime = 0.719, 
                                   data_flx$lat, data_flx$long)

data_flx_month <- data_flx %>% cbind(ppfd_sun = ppfd_sun,ppfd_shade = ppfd_shade) %>% 
...
res <- rpmodel_sun_shade(
  data_flx_month$Tair,
  tcleaf = NA,
  data_flx_month$VPD*100,
  data_flx_month$CO2,
  fapar = 1,
  data_flx_month$ppfd_sun,
  data_flx_month$ppfd_shade,
  patm = data_flx_month$PA*1000,
  elv = NA,
  beta = 146,
...
res_original <- rpmodel_core(
  data_flx_month$Tair,
  tcleaf = NA,
  data_flx_month$VPD*100,
  data_flx_month$CO2,
  fapar = data_flx_month$FAPAR,
  data_flx_month$PPFD,
  patm = data_flx_month$PA*1000,
  elv = NA,
  beta = 146,
...
res_original_Ic <- rpmodel_core(
  data_flx_month$Tair,
  tcleaf = NA,
  data_flx_month$VPD*100,
  data_flx_month$CO2,
  fapar = 1,
  data_flx_month$ppfd_sun + data_flx_month$ppfd_shade,
  patm = data_flx_month$PA*1000,
  elv = NA,
  beta = 146,
  c_cost = 0.41, # Change to 0.05336251 if method_jmalim = "smith19" is used
...
df_res <- as_data_frame(res) %>% 
  cbind(as_data_frame(res_original),
        `I sun` = data_flx_month$ppfd_sun,
        `I shade` = data_flx_month$ppfd_shade,
        `I c` = data_flx_month$PPFD*data_flx_month$FAPAR,
        month = lubridate::month(data_flx_month$month, label=FALSE))



p1 <- df_res %>% 
  pivot_longer(c(`vcmax25_sun`,`vcmax25_shade`,`vcmax25`),names_to = 'Model') %>% 
  ggplot()+
  geom_line(aes(month,value,color=Model), linewidth = 1)+
  theme_bw()+
  xlab("Month")+
...
p2 <- df_res %>% 
  pivot_longer(c(`gpp_sun`,`gpp_shade`,`gpp`),names_to = 'Model') %>% 
  ggplot()+
  geom_line(aes(month,value,color=Model), linewidth = 1)+
  geom_line(data=df_res,aes(month,gpp_sun+gpp_shade),linetype=2)+
  # geom_line(data=data_flx_month,aes(month,GPP*12.0107),linetype=2)+
  theme_bw()+
  xlab("Month")+
  ylab(expression("GPP ["*mu*"g"~m^{-2}~s^{-1}*"]"))+
  theme(legend.text = element_text(size = 12))+
  scale_x_continuous(breaks = seq(1,12,1))+
  # scale_x_datetime(labels = date_format("%b"))+
  NULL
...
p3 <- df_res %>% 
  pivot_longer(c(`I sun`,`I shade`,`I c`),names_to = 'Irradiance model') %>% 
  ggplot()+
  geom_line(aes(month,value,color=`Irradiance model`), linewidth = 1)+
  theme_bw()+
  xlab("Month")+
