...
resistance_neutral <- function(ws_mean=ws_mean, canopy_height = canopy_height){
  ...
  z = canopy_height+2 #wind measure height, we assume that the wind measure is 2 m over plant height
  b =  0.6
  a = 0.1
  #Aerodynamic conductance
  gaM = k^2 * ws_mean/
    (log((z-canopy_height*b)/(canopy_height*a))*log((z-canopy_height*b)/(0.135*canopy_height*a))) #aerodynamic conductance for momentum; Tan et al.2019 (eq. 14) + Chu et al. 2018 (Supporting information)
  
  #boundary layer conductance
  kB1 = 2 #Tan et al. 2019 eq. 20 
  u_friction = (k*ws_mean)/(log((z-canopy_height*b)/(canopy_height*a))+log(1.25)) #friction velocity
...
calc_ustar <- function(ws,canopy_height,z,LAI){
  ...
  ust_calc <- (k*u)/(log((z-Hc*alpha_2)/(Hc*alpha_1))+log(lamb_rs))
...
calc_ga <- function(ws,ust,canopy_height, tcleaf_root,
  ...
  u <- ws
  if(u <=0){u <- 0.01} #avoid infinite atmospheric stability index
  z <- z # measurement height (m)
  Hc <- canopy_height # mean canopy height (m)
...
calc_air_density <- function(patm, tc){
